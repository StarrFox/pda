...
from .listdb.github_issues import GithubIssues
...
def controller(_db):
    ...
    p.add_argument(
        "listname", nargs="?", type=str, help="tell pda which list to work on"
    )
    ...
    # ================#
    p.add_argument(
        "--start", action="store_true", help="start pda and sync with remote"
    )

    p.add_argument("--stop", action="store_true", help="stop pda and sync with remote")

    # option to close all the tasks from todo list
    p.add_argument("--clear", action="store_true", help="remove all the tasks in list")
