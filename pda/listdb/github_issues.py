...
class GithubIssues:
    ...
    def __init__(self, config):
        ...
        self._shelf = shelve.open(
            os.path.abspath(config.local_db_path), protocol=-1, writeback=True
        )
        self._remote_mode = config.remote_mode
        self._local_dbpath = config.local_db_path

        if config.remote_mode:
            repo_name = config.username + "/" + config.reponame
            self._url_issues = self.BASE_URL + repo_name + "/issues"
            self._url_milestones = self.BASE_URL + repo_name + "/milestones"
            self._url_labels = self.BASE_URL + repo_name + "/labels"
            self._auth = (config.authtoken, "")
        else:
            self._url_issues = ""
            self._url_milestones = ""
            self._url_labels = ""
            self._auth = ("", "")
    ...
    def max_task_number(self):
        ...
        if self.shelf:
            task_numbers = [
                int(task_no) for task_no in self.shelf if task_no != "CMDS_HISTORY"
            ]
            if task_numbers:
    ...
    def _is_selected(self, task_no, task_type, milestone, priority):
        ...
        return (
            (task_type is None or task_type == task_type_in_shelf)
            and (milestone is None or milestone == milestone_in_shelf)
            and (priority is None or priority == priority_in_shelf)
        )
    ...
    def _is_cmd_history_annihilable(self, task_number):
        ...
        n_cmd_list = [
            ...
            if not (cmd["#"] == task_number and cmd["CMD"] != "REMOVE")
        ]
    ...
    def _is_added_issue_updated(
        self, task_number, new_summary, new_tasktype, new_milestone, new_priority
    ):
    ...
    def _update_labels(self, cmd):
        ...
        resp = requests.get(
            self.url_issues + "/" + str(issue_number) + "/labels", auth=self.auth
        )
    ...
    def _get_payload_for_add_or_edit(self, cmd, payload):
        ...
        labels = (
            self._get_labels(cmd) if cmd["CMD"] == "ADD" else self._update_labels(cmd)
        )
    ...
    def _print_task(self, task_number, task_type, milestone, priority):
        ...
        if self._is_selected(task_number, task_type, milestone, priority):
            ...
            if self.shelf[task_number]['summary']:
                msg += f"{self.shelf[task_number]['summary']:<60}  "

            else:
                msg += " " * 62

            if self.shelf[task_number]['type']:
                msg += f"{self.shelf[task_number]['type']:<9}  "

            else:
                msg += " " * 11

            if self.shelf[task_number]['milestone']:
                msg += f"{self.shelf[task_number]['milestone']:<8}  "

            else:
                msg += " " * 10

            if self.shelf[task_number]['priority']:
                msg += f"{self.shelf[task_number]['priority']:<8}"
    ...
    def extend_milestone(milestone):
        ...
        return (
            {"d": "day", "w": "week", "m": "month", "s": "season", "y": "year"}.get(
                milestone, None
            )
            if milestone
            else None
    ...
    def edit_task(
        ...
        if self.has_task(task_number):
            ...
            if self.remote_mode and not self._is_added_issue_updated(
                task_number, new_summary, new_tasktype, new_milestone, new_priority
            ):
                cmd_history_data = {
                    "CMD": "EDIT",
                    "#": task_number,
